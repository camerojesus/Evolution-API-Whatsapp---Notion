...
//enviarMensajeDeRetorno(client, "5851192238", "texto de prueba");
...
async function enviarMensajeDeRetorno(client, numeroDestino, texto) {
  try {
      ...
      await client.sendMessage(numeroDestino, texto);
      log(`Mensaje enviado a ${numeroDestino}: ${texto}`);
  } catch (error) {
...
client.on('ready', () => {
  ...
  bloquearContactos(client); // Llamada a la función para bloquear contactos
});
