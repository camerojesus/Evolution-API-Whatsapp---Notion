...
// Fallback opcional a qrcode-terminal (más estable en algunas consolas)
let qrcodeTerminal = null; try { qrcodeTerminal = require('qrcode-terminal'); } catch {}
const fs      = require('fs');
...
if (!DEFAULT_NOTION_API_KEY || !DEFAULT_PAGE_ID) {
  console.error('Advertencia: NOTION_API_KEY o PAGE_ID no definidos en .env. Configure el archivo .env con sus claves reales.');
  console.log('El QR aparecerá una vez configuradas las variables.');
  // No salir para mostrar QR
}

/*━━━━━━━━━━  CONFIGURACIÓN DE CONSOLA  ━━━━━━━━━━*/
const bConsola = true;            // true = muestra logs y QR
const FORCE_NEW_SESSION = /^(1|true|yes)$/i.test(process.env.FORCE_NEW_SESSION||'');

/*━━━━━━━━━━  DIRECTORIO BASE PARA SESIÓN Y LOGS  ━━━━━━━━━━*/
const baseDataDir = path.join(__dirname, 'data');
if (!fs.existsSync(baseDataDir)) fs.mkdirSync(baseDataDir, { recursive: true });
// Ruta donde LocalAuth almacenará la sesión (carpeta Session-* dentro de baseDataDir)
log(`Ruta base de datos/sesión: ${baseDataDir}`);

// Si se solicita forzar nueva sesión, eliminar carpetas de sesión previas
if (FORCE_NEW_SESSION) {
  try {
    const entries = fs.readdirSync(baseDataDir);
    let removed = 0;
    for (const e of entries) {
      if (/^session|^Session|^wwebjs/i.test(e)) { // patrones comunes
        const full = path.join(baseDataDir, e);
        fs.rmSync(full, { recursive: true, force: true });
        removed++;
      }
    }
    log(`FORCE_NEW_SESSION activo. Carpetas de sesión eliminadas: ${removed}`);
  } catch (e) {
    log(`Error eliminando sesiones previas: ${e.message}`, true);
  }
}
...
for(const {proyecto,apiKey,databaseId} of proyectosNotion){
  notionIntegrations[proyecto.toLowerCase()] = {
    client: new NotionClient({ auth: apiKey }),
    databaseId
  };
}
const defaultNotion = {
  client: new NotionClient({ auth: DEFAULT_NOTION_API_KEY }),
...
async function addEntryToNotion(
  ...
  try{
    ...
    log('Error creando fila Notion: '+e.message,true);
    return;
  }
  ...
  let subId;
  try{
    const r = await notionClient.pages.create({
    ...
    log('Error creando sub-página: '+e.message,true);
    return;
  }
  ...
  try{
    ...
    log('Error actualizando mención: '+e.message,true);
  }
}
...
client.on('authenticated',()=>log('Autenticado (evento authenticated).'));
...
/*━━━━━━━━━━  INIT  ━━━━━━━━━━*/
if (!FORCE_NEW_SESSION) {
  // Intentar detectar si ya existe alguna carpeta de sesión para informar que quizá no saldrá QR
  try {
    const existing = fs.readdirSync(baseDataDir).filter(e=>/^session|^Session|^wwebjs/i.test(e));
    if (existing.length>0) {
      log(`Sesión existente detectada (${existing.join(', ')}). Si no aparece QR y desea regenerarlo, establezca FORCE_NEW_SESSION=true.`);
    } else {
      log('No se han encontrado carpetas de sesión previas. Se espera que aparezca un QR.');
    }
  } catch {}
}
